...
# find the keypoints and descriptors with SIFT
    ...
    # print("matches",np.array(matches).shape)
...
with open("cam_params.txt","w") as f1:
    for j in range(1):
        img1 = cv2.imread('/home/divanshu05/7thsem/MTP/3Dreconstruction/images/viking/img{}.jpg'.format(1))   
        # img1 = cv2.resize(img1, (0,0), fx=.2, fy=.15) 
        # print(img1.shape)
        gray1 = cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY)
        img2 = cv2.imread('/home/divanshu05/7thsem/MTP/3Dreconstruction/images/viking/img{}.jpg'.format(j+2))
        # img2 = cv2.resize(img2, (0,0), fx=.2, fy=.15) 
        gray2 = cv2.cvtColor(img2, cv2.COLOR_BGR2GRAY)
        # print(img2.shape)
        # gray2 = cv2.imread("/home/divanshu05/7thsem/MTP/3Dreconstruction/images/3_pen/img{}.jpg".format(j+2),0)
        # gray1 = cv2.imread("/home/divanshu05/7thsem/MTP/3Dreconstruction/images/3_pen/img{}.jpg".format(1),0)
...
with open('data.txt','w') as f:
    for k in range(3,n_cameras+1):
        img2 = cv2.imread('/home/divanshu05/7thsem/MTP/3Dreconstruction/images/viking/img{}.jpg'.format(k-1))
        img3 = cv2.imread('/home/divanshu05/7thsem/MTP/3Dreconstruction/images/viking/img{}.jpg'.format(k))
        ...
        for i,item in enumerate(tup):
            ...
            if(str(tup[i][0]) not in ind2):
                indices2.append(tup[i][0])
            ind2.add(str(tup[i][0]))
            if(str(tup[i][1]) not in ind1):
                indices1.append(tup[i][1])
            ind1.add(str(tup[i][1]))
        ...
        impoints = [p_k1[i] for i in indices2]
        impoints = np.array(impoints)
        p_k3 = np.vstack((impoints,p2.T)).T
...
def project(points, camera_params):
    ...
    f = 523.81*np.ones(camera_params.shape[0])
    # np.ones()
...
def fun(params,n_cameras,n_points,camera_indices,point_indices,points_3d , points_2d):
    ...
    #     r = camera_params[camera_indices[i]][0:3]
    #     R,_ = cv2.Rodrigues(r)
    #     t = np.reshape(np.array(camera_params[camera_indices[i]][3:6]),(3,1))
    #     pose = np.hstack((R,t))
    #     proj = np.dot(np.dot(K,pose),np.hstack((points_3d[point_indices[i]],1)).T)
    #     proj = proj/proj[2]
    #     points_proj.append(proj[0:2])
    # points_proj = np.reshape(np.array(points_proj),(point_indices.shape[0],2))    
